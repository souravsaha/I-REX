...
public class CompareCommand extends Commands{
	...
	public CompareCommand(LucDebObjects lucivObjects) {
	...
	public int findRank(int luceneDocid, String queryTerms[]) throws IOException
	{
        ...
        try {
            ...
            if(rankedList.containsKey((lucdebObjects.getIndexSearcher().doc(luceneDocid).get(lucdebObjects.idField))))
    ...
    public void execute(String[] args, PrintStream out) throws IOException {
        ...
        if(cmd.hasOption("i1"))
        {
	        ...
	        try {
	            luceneDocid1 = Integer.parseInt(luceneDocIdNum1.trim());
        ...
        else if(cmd.hasOption("n1"))
        {
        	try {
                ...
                luceneDocid1 = lucdebObjects.getLuceneDocid(docNameValue1.trim());
            } catch (Exception ex) {
        ...
        if(cmd.hasOption("i2"))
        {
	        // Parsing the arguments
	        try {
	            luceneDocid2 = Integer.parseInt(luceneDocIdNum2.trim());
	        } catch(NumberFormatException ex) {
	            out.println("error reading docid; expected integers.");
	        }
        }
        else if(cmd.hasOption("n2"))
        {
        	try {
                ...
                luceneDocid2 = lucdebObjects.getLuceneDocid(docNameValue2.trim());
            } catch (Exception ex) {
