...
//        this.analyzer = rblm.analyzer;
        ...
        this.QMIX = rblm.QMIX;
        vocSize = getVocabularySize(indexReader, fieldForFeedback);
...
//                    feedbackTermStats.put(key, new PerTermStat(key, value.getCF(), value.getDF()));
    ...
    /**
     * Used Dirichlet smoothed MLE:
     *      tf + mixingMu * cf /(float)vocSize / (float)( docsize + mixingMu )
     * 
    ...
    public float return_Smoothed_MLE(String t, DocumentVector dv) throws IOException {
        ...
        if (colPTS != null) {
            float tf = (float) ((docPTS!=null)?(float)docPTS.getCF():0);
            float cf = (float)feedbackTermStats.get(t).getCF();
            float docsize = dv.getDocSize();

            smoothedMLEofTerm = ( tf + mixingMu * cf /(float)vocSize / (float)( docsize + mixingMu ));
        }
...
//        long vocSize = terms.getSumTotalTermFreq();  // total number of terms in the index in that field
        ...
        vocSize = indexReader.getSumTotalTermFreq(field);
        return vocSize;
    }
    ...
    public HashMap RM1(TopDocs topDocs) throws Exception {
        ...
        Collections.sort(list_PwGivenR, (WordProbability t, WordProbability t1) -> t.p_w_given_R<t1.p_w_given_R?1:t.p_w_given_R==t1.p_w_given_R?0:-1);
    ...
    public BooleanQuery getExpandedQuery(HashMap<String, WordProbability> expandedQuery, TRECQuery query) throws Exception {
        ...
        for (Map.Entry<String, WordProbability> entrySet : expandedQuery.entrySet()) {
            ...
            booleanQuery.add(new BoostQuery(tq, value), BooleanClause.Occur.SHOULD);
        }
